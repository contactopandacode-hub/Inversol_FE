...
namespace ServicioRSNetCore.Controllers.Funciones
{
    ...
    decimal ldc_montoFlete, decimal ldc_montoSeguro, decimal ldc_montoGastosDestino
        ...
        public string ObtenerAdjunto(string str_identificador, string str_ServicioUsuario, string str_ServicioClave, string str_Url, string str_filePath, string str_tipo)
        {
            string str_Resultado = string.Empty;
            try
            {
                using (var obj_Cliente = new HttpClient())
                {
                    ...
                    using (var pdfClient = new HttpClient())
                    {
                        ...
                        if (str_tipo == "1")
                            pdfUrl = str_Url + "/v1/pdf/" + str_identificador;

                        if (str_tipo == "2")
                            pdfUrl = str_Url + "/v1/xml/" + str_identificador;

                        if (str_tipo == "3")
                            pdfUrl = str_Url + "/v1/cdr/" + str_identificador;

                        var pdfResponse = pdfClient.GetAsync(pdfUrl).Result;
                        pdfResponse.EnsureSuccessStatusCode();

                        byte[] pdfBytes = pdfResponse.Content.ReadAsByteArrayAsync().Result;

                        File.WriteAllBytes(str_filePath, pdfBytes);
            ...
            {
                str_Resultado = "01|Error al descargar el Adjunto: " + ex.Message;
            }
            catch (Exception ex)
            {
                str_Resultado = "02|Error no controlado: " + ex.Message;
            }
